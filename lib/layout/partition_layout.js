...
let setupFunction = function () {
    ...
    $(window).mouseup(function (e) {
        ...
        if (target_border) {
            ...
            if (brother && (isInside(target, pageX, pageY) || isInside($(brother[1]), pageX, pageY))) {
                ...
                $("#" + selected_node.id).trigger("layout:resize");
     ...
     * @param {object} node_selection node mesmo objeto porem seleção jquery
     * @param {} current_dir - poionamento para criar filhos ou irmãos ao elemento
     * @param {*} brother  -elemento pai para verificações
     */

    function addNode(node_data, node_selection, current_dir, brother,width,height) {
        ...
        let html_node = $("<div/>")
            ...
            .append($("<button/>")
                ...
                .click(function(){
                    alert("foi",this);
                    let node =  $(this).parent().get(0).__node__;
                    let nodeParent = node.parent;
                    
                    

                    addNode(nodeParent,$("#"+nodeParent.id),nodeParent.dir,node);
