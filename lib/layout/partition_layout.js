...
class DynamicPartition{
     ...
     */
    addNode(node_data,current_dir = node_data.dir, brother) {
        ...
        let html_node = $("<div/>")
          ...
          .append($("<button/>")
            ...
            .click((elemtent)=>{
                ...
                let node =  $('.addNode').parent().get(0).__node__;
                let nodeParent = node.parent;

                console.log("dir:",nodeParent.dir);

                console.log("node:",node);
                console.log("parent:",nodeParent);

                this.addNode(nodeParent,nodeParent.dir,node);
    ...
    mouseup(){
        $(window).mouseup( (e)=> {
            if (this.target_border) {
                ...
                let rootOffset = this.$partition_root.offset();
                let pageX = this.putInside(e.pageX, rootOffset.left, rootOffset.left + this.$partition_root.width());
                let pageY = this.putInside(e.pageY, rootOffset.top, rootOffset.top + this.$partition_root.height());
    ...
    * */
    mousedown(){
        ...
        this.$partition_root.attr("id", this.parent_id).on("mousedown", ".partition-divisor",  (e)=> {
            ...
            this.highlight_line_div.addClass($target.attr("class"));
            ...
            $target.parent().get(0).onmousemove =  (e2)=> {
