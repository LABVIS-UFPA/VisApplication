...
class PartitionLayout {
    ...
    constructor(html_container) {
        ...
        $(this.html_container).on('click', '.partition-node', function (e) {
            ...
            let location = ph.p_dir === 'column' ? $(e.target).offset().top : $(e.target).offset().left
            if (node.layout.cutter_on && partition) { //&& this === partition){
                if (node === partition && partition.isLeaf || !partition.isLeaf && partition.hasChild(node)) {
                    if (ph.p_dir !== partition.direction) {
                        ...
                        location = ph.p_dir === 'column' ? $(e.target).offset().top : $(e.target).offset().left
                    }
                    console.log("localtion:",location);

                    let dif_proportion = ph.p_dir === 'column' ? location / partition.height : location / partition.width;

                    ph.setLocation(location)
                    console.log(" proprotion : ",dif_proportion);
                    console.log(" proptortion -1: ",1 - dif_proportion)

                    // caso 1
                    //--- partições para leaf tratar mudança de content do - parent para o conteudo ----
                    //metade pra cima metade para
                    if (partition.isLeaf) {
                        partition.addChild(new Partition(node.layout, {proportion: dif_proportion}), false);
                        partition.addChild(new Partition(node.layout, {proportion: 1 - dif_proportion}));
                        ...
                        node.proportion = dif_proportion * nodeprop;

                        partition.addChild(new Partition(node.layout, {proportion: nodeprop * (1 - dif_proportion)}), true, indexNode);
...
class Partition {
    ...
    constructor(layout, {
        ...
        this.html = $('<div/>').attr('id', 'partition-' + this.id)
            ...
            .mousemove((e) => {
                ...
                //         console.log(partition.hasChild(this))
                //         console.log(partition,this)
                //
                //         if (this === partition && partition.isLeaf || !partition.isLeaf && partition.hasChild(this)) {
                ...
                //             }
                //
                //             console.log("par", partition)
                //
                //             let dif = ph.p_dir === 'column' ? location / partition.height : location / partition.width;
                //
                //             ph.setLocation(location)
                //
                //             //metade pra cima metade para
                ...
                //             if (partition.isLeaf) {
                //                 // console.log('dif',dif);
                //                 partition.addChild(new Partition(this.layout, {proportion: dif}), false);
                //             }
                //
                //             partition.addChild(new Partition(this.layout, {proportion: 1 - dif}));
                //             // console.log('dif -1',1-dif)
                //             //   partition.layout.updateLayout();
                //             partition.layout.updateHTML();
                //
                //         }
     ...
     */
    addChild(partition, updateProportion = true, index = Infinity) {
        ...
        if (this.children.length > 1) {
            ...
            $(this.html.childNodes[index]).after(this.divisors[lasti].html)
        }
