...
class PartitionLayout {
            ...
            if (node.layout.cutter_on && partition) { //&& this === partition){
                if (node === partition && partition.isLeaf || !partition.isLeaf && partition.hasChild(node)) {
                    ...
                    if (partition.isLeaf) {
                        ...
                        if (content_svg) {
                            partition.addChild(new Partition(node.layout, {proportion: dif_proportion}), false, Infinity, content_svg,true);
                            $(partition.html).children().remove();
                            $(partition.html).children("div").remove();
                        } else {
                            partition.addChild(new Partition(node.layout, {proportion:  dif_proportion}), false, Infinity,undefined,true);
                        }
                        partition.addChild(new Partition(node.layout, {proportion:1 - dif_proportion}), false,Infinity,undefined);

                        if (ph.p_dir === 'row') {
                            this.divisors = [new Divisor(partition.children[0], partition.children[1])]
                            $(partition.html.childNodes[0]).after(this.divisors[0].html)
                        }
                        ...
                        partition.addChild(new Partition(node.layout, {proportion: nodeprop * dif_proportion}), true, indexNode,undefined,true);
    ...
    async import(json) {
        let self = this
        const add_child_tree = async function (item, item_json, index) {

            if (item_json.children) {
                await this.addChild(item, true, Infinity, "")
            } else {
                await this.addChild(item, true)
            }
            ...
            for (const [index, child] of item_json.children.entries()) {
                await add_child_tree.call(item, new Partition(self, child), child)
            }
        }
        ...
        for (const [index, child] of json.children.entries()) {
            await add_child_tree.call(this.root, new Partition(this, child), child, index)
...
class Partition {
     ...
     * @param imported init bolean para verificar se o conteudo é inicial da div
     */
    addChild(partition, updateProportion = true, index = Infinity, content, init = false) {
...
class PlaceHolderDivisor {
    ...
    constructor(p_dir) {
        ...
        this.html = $('<div/>').addClass('partition-divisor')
            .addClass(p_dir === 'row' ? 'partition-divisor-ver' : 'partition-divisor-hor')
            .css(p_dir === 'row' ? 'border-left' : 'border-top', '2px dashed gray')
