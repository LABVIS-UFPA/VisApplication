  ...
  import (json) {
    ...
    const add_child_tree = function (item, item_json) {
      ...
      for (let i = 0; i < item_json.children.length; i++) {
        add_child_tree.call(item, new Partition(self, item_json.children[i]), item_json.children[i])
      }
    ...
    for (let i = 0; i < json.children.length; i++) {
      add_child_tree.call(this.root, new Partition(this, json.children[i]), json.children[i])
    }
...
class Partition {
  ...
  constructor (layout, json = {
    ...
    this.proportion = json.proportion
  ...
  addChild (partition) {
    ...
    if (this.children.length > 1) {
      ...
      for (let i = 0; i < this.divisors.length; i++) {
        // console.log('i', this.divisors[i].resizeDivisions(false))
        this.divisors[i].resizeDivisions(false)
      }
